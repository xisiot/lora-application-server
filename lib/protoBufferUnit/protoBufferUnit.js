...
function ProtoBufUnit(log, mysqlConn) {
  this.log = log;
  this.root = new protobuf.Root();
  this.App2PB = mysqlConn.App2PB;
  this.loadAllData();
}

ProtoBufUnit.prototype.loadAllData = function () {
  console.log('----------------load all pb data-------------------------');
  this.App2PB.readItems().then(res=>{
    let rows = res.rows;
    let pb = {};
    let message;
    this.root = new protobuf.Root();
    try {
      for (var i in rows) {
        message = JSON.parse(rows[i].dataValues.message);
        Object.keys(message).map((value) => {
          message[value].id = parseInt(message[value].id);
        });
        console.log(message);
        pb.nested = {};
        pb.nested[rows[i].dataValues.AppEUI] = {
          nested: {
            message: {
              fields: message
            }
        ...
        protobuf.Root.fromJSON(pb, this.root);
      }
    }catch(err) {
      console.log(err);
    }
